...
def remove_html_comments(html_content):
    ...
    if gpl_comment_match:
        ...
        flexible_gpl_pattern = r'^(<!--\s*[\s\S]*?2A Toolbox Arsenal Management System[\s\S]*?GNU General Public License[\s\S]*?-->)(.*)'
...
def lightly_minify_html(html_content):
    ...
    gpl_pattern = r'^(<!--\s*\n// 2A Toolbox Arsenal Management System[\s\S]*?-->)(.*)'
